...
export default async function TestsPage({ searchParams }: { searchParams: Promise<{ page: string, limit: string }> }) {
    ...
    const tests = await fetch(`${API_URL}${API_ROUTES.TESTS}?page=${pageNumber}&limit=${limitNumber}`, {
        headers: {
            ...
            <TableBody>
                {testsData.data.map((test) => (
                    <TableRow key={test.id}>
                        ...
                        <TableCell>{dayjs(test.createdAt).format('DD.MM.YYYY HH:mm')}</TableCell>
                        <TableCell>
                            <Link
